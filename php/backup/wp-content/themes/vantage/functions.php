...
if (!function_exists('vantage_render_slider')) :
    ...
    function vantage_render_slider() {
        ...
        if (is_front_page() && !in_array(siteorigin_setting('home_slider'), array('', 'none'))) {
            ...
            echo do_shortcode("[ultimate-slider]");
        } else {
            $page_id = get_the_ID();
...
if (!function_exists('vantage_footer_site_info_sub')) :
    ...
    function vantage_footer_site_info_sub($copyright) {
        ...
        return str_replace(
                array('{site-title}', '{copyright}', '{year}'), array(get_bloginfo('name'), '&copy;', date('Y')), $copyright
        );
    }
...
add_action('init', 'my_plugin_menu');
/*
  function lar_legal() {

  $labels = array(
  'name' => _x('Lar Legal', 'post type general name'),
  'singular_name' => _x('Audio', 'post type singular name'),
  'add_new' => _x('Adicionar', 'Audio item'),
  'add_new_item' => __('Adicionar Audio'),
  'edit_item' => __('Editar Audio'),
  'new_item' => __('Novo Audio'),
  'view_item' => __('Ver Audio'),
  'search_items' => __('Procurar Audio'),
  ...
  register_post_type('lar_legal', $args);
  }

  add_action('init', 'lar_legal'); */

add_action('admin_menu', 'addCustomMenuItem');

function addCustomMenuItem() {

    add_menu_page('Lar legal', 'Projeto Lar Legal', 'manage_options', 'lar_legal_morettic_com_br', 'pg_building_function', '', 3);
}
...
function pg_building_function() {
    ...
    //
    include_once '/home/manguevi/public_html/wp/wp-content/themes/vantage/grid.php';
