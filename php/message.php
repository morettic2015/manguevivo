    ...
    <body>
        <div data-role="page" data-dialog="true">
            ...
            if (isset($_GET['action'])) {
                ...
                VALUES (
                NULL , 
                CURRENT_TIMESTAMP ,  '1071267682998806',  'Mangue vivo[Nova Mensagem]:" . $_GET['message'] . "',  '" . $_GET['id'] . "')";
                @$result = $db->query($query);


                $query = "SELECT (SELECT sml_email FROM wp_sml WHERE id =  `fk_wp_sml` ) FROM  `wp_sml_lar_legal` WHERE id =" . $_GET['id'];
                $result = $db->query($query);
            ...
            ?>
            <form action="http://manguevivo.org.br/wp/wp-content/themes/vantage/message.php" method="get" onsubmit="return  enviar();
                    void(0);">
                <div role="main" class="ui-content">
                    <label>Mensagem<textarea name="message"></textarea></label>
                </div>
                <input type="submit" value="Enviar mensagem"/>
                <input type="hidden" value="<?php echo $_GET['id']; ?>" name="action"/>
